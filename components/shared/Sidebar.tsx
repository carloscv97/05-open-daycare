...
function NavigationIcon({ icon }: { icon: string }) {
  if (icon === 'home')
    return <path d="M3 9.5 12 3l9 6.5V20a1 1 0 0 1-1 1h-5v-6H9v6H4a1 1 0 0 1-1-1z" />;
  if (icon === 'children')
    return (
      <>
  ...
  return (
    <aside className="sticky top-0 hidden h-screen w-[248px] shrink-0 flex-col border-r border-border bg-surface px-4 py-6 lg:flex">
      <div className="flex items-center gap-[11px] px-2 pb-[22px] pt-1">
        ...
        <div>
          <p className="font-display text-[17px] font-semibold leading-none text-text">
            OpenDayCare
          </p>
          <p className="mt-0.5 text-[11.5px] text-text-faint">Sala Soles</p>
        </div>
      </div>
      <button
        type="button"
      ...
      <div className="mt-2.5 border-t border-border pt-3.5">
        <div className="flex items-center gap-[11px] px-2 py-1.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-extrabold text-text">Caro Giménez</p>
            <p className="text-xs text-text-faint">Maestra · Soles</p>
          </div>
          <span className="flex h-8 w-8 items-center justify-center rounded-[10px] bg-canvas text-text-muted">
            <svg
              aria-hidden="true"
              className="h-4 w-4"
              viewBox="0 0 24 24"
